...
| [法律聚合记录引用](../website/src/content/docs/architecture/culture-law.mdx) | legal aggregate record reference | `LegalRecordRef` | The exact kind and identity of a record inside `LegalRuntimeRecord`; it is deliberately not a kernel `DomainRecordVersionRef` because law-local members are committed as one aggregate. |
