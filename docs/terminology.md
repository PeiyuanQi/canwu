...
| [文化系统 SDK](../website/src/content/docs/architecture/culture-law.mdx) | culture system SDK | `canwu-culture` | An experimental optional authoring, compilation, and lifecycle extension above `canwu-society`; it validates content and produces a deterministic execution plan without becoming a core subsystem. |
| [法律制度化](../website/src/content/docs/architecture/culture-law.mdx) | legal institutionalization | `canwu-law` (proposed) | A downstream extension that turns admitted cultural signals into controller-mediated legal proposals and versioned law records; culture never writes legal state directly. |
| [法律版本](../website/src/content/docs/architecture/culture-law.mdx) | law version | `LawVersion` (proposed) | An immutable, versioned legal rule record with jurisdiction, authority, effective interval, causal references, and enforcement metadata. |
