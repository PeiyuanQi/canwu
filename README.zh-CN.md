...
目前的 v0.2 开发版本在保留小型移动场景的同时，开始加入《社稷》引擎符合性
合同。示例世界包含两个人、一个政府、一支军队、三个领地和两条路线。
指挥者可以命令军队移动。引擎会验证命令、安排旅程、推进时间，并在军队到达
后更新位置和记录事件。指挥者会立即知道军队已经到达，另一个人则会保留旧
信息，直到报告传到他那里。
...
Canwu 使用语义化版本。工作区中的所有 crate 当前版本都是 `0.2.0`，并会使用
