(() => {
  ...
  async function loadDefaultTrace() {
    try {
      setStatus("读取默认样例…");
      const base = new URL("../../artifacts/traces/ming-fiscal-reference/hongwu-1391/", window.location.href);
      const [manifestResponse, stepsResponse] = await Promise.all([
        fetch(new URL("manifest.json", base)),
        fetch(new URL("steps.jsonl", base)),
      ]);
      if (!manifestResponse.ok || !stepsResponse.ok) throw new Error("默认样例不存在；请先运行 starter 生成 trace，或选择本地文件。");
      ...
      await readTraceFiles(manifest, steps, "默认样例");
    } catch (error) {
  ...
  function renderOverview() {
    ...
    $("statTime").textContent = last ? String(last.at) : "—";
    ...
    nodes.timeline.innerHTML = visible.map((frame) => `
      <button class="timeline-row" type="button" data-sequence="${frame.sequence}" aria-pressed="${frame.sequence === state.selectedSequence}">
        ...
        <span class="timeline-main">
          ...
          <span class="timeline-time">t=${escapeHtml(frame.at)} · boundary=${escapeHtml(frame.boundaryId)}</span>
        </span>
    ...
    nodes.detail.innerHTML = `
      <div class="detail-heading">
        <div>
          ...
          <p><code>${escapeHtml(frame.boundaryId)}</code> · 模拟时间 <code>${escapeHtml(frame.at)}</code> · revision <code>${escapeHtml(frame.revision)}</code></p>
        </div>
